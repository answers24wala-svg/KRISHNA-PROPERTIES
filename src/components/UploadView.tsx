...
export default function UploadView({ 
  ...
  const [propertyType, setPropertyType] = useState<'Apartment' | 'Independent Villa' | 'Penthouse' | 'Commercial' | 'Flat' | 'Bungalow' | 'PG'>(editingProperty?.propertyType || 'Apartment');
  ...
  // Contact Info
  const [ownerName, setOwnerName] = useState(editingProperty?.agent?.name || 'Gopal Naidu');
  const [phoneNumber, setPhoneNumber] = useState(editingProperty?.agent?.phone || '9638177321');
